...
#             outputs = (loss,) + logits_cos 
            ...
            return final_logits, logits_ce, cop_logits_ce, ocp_logits_ce, ocop_logits_ce
...
#             print(logits[0])
            ...
            logits_cp = logits_cp.detach().cpu().numpy()
            logits_cop = logits_cop.detach().cpu().numpy()
            logits_ocp = logits_ocp.detach().cpu().numpy()
            logits_ocop = logits_ocop.detach().cpu().numpy()
#             print(logits)
...
#             print(label_ids)
            ...
            raw_score += zip(logits, logits_cp, logits_cop, logits_ocp, logits_ocop, pred_label, label_ids)
...
#             writer.write(classification_report(gold_labels, predicted_labels, target_names=label_list, digits=4))
        ...
        with open(output_raw_score, 'w') as fout:
            fields = ["undermine_score", "support_score","undermine_cp","support_cp","undermine_cop","support_cop","undermine_ocp", "support_ocp","undermine_ocop", "support_ocop", "predict_label", "gold"]
            writer = csv.DictWriter(fout, fieldnames=fields)
            writer.writeheader()
            for score, cp_socre, cop_score, ocp_score, ocop_score, pred, gold in raw_score:
                writer.writerow({
                    "undermine_score": str(score[0]),
                    "support_score": str(score[1]),
                    "undermine_cp": str(cp_socre[0]),
                    "support_cp": str(cp_socre[1]),
                    "undermine_cop": str(cop_score[0]),
                    "support_cop": str(cop_score[1]),
                    "undermine_ocp": str(ocp_score[0]),
                    "support_ocp": str(ocp_score[1]),
                    "undermine_ocop": str(ocop_score[0]),
                    "support_ocop": str(ocop_score[1]),
